...
bot = telebot.TeleBot('1432717319:AAE_qoD6coUgmcT5CTQAb-jqP0Q2KtV_B20')
bot.remove_webhook()
...
def query_handler(call):
    if call.data == 'japanese_horoscope':
        ...
        res = requests.get(
            'https://orakul.com/horoscope/japanese/mouse.html').text
        soup = BeautifulSoup(res, features="html.parser")
        soup.encode('utf-8')
        cases = soup.find("p", {"class": ""})
        info = cases.getText()
        bot.send_message(
            call.message.chat.id, f'Ваш гороскоп: *год Мыши* \n{info}', parse_mode='Markdown')
        ...
        res = requests.get(
            'https://orakul.com/horoscope/japanese/snake.html').text
        soup = BeautifulSoup(res, features="html.parser")
        soup.encode('utf-8')
        cases = soup.find("p", {"class": ""})
        info = cases.getText()
        bot.send_message(
            call.message.chat.id, f'Ваш гороскоп: *год Змеи* \n{info}',parse_mode='Markdown')
